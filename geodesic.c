...
#define PRINTVALS() { \
  	double vals[] = {lat1, lon1, lat2, lon2, s12, m12, M12, M21, S12, lon12, lon12s,\
		sbet1, cbet1, sbet2, cbet2, s12x, m12x, dn1, dn2, lam12, slam12,clam12, \
    a12, sig12, calp1, salp1, calp2, salp2, \
    Ca[0], Ca[1], Ca[2], Ca[3], Ca[4], Ca[5], Ca[6], omg12, somg12,\
		comg12};\
    printvals(vals, sizeof(vals)/sizeof(double));\
}

void printvals(double *vals, int nvals) {
  double sum = 0.0;
  printf("cc:");
  ...
  if (!meridian &&
    ...
    if (sig12 >= 0) {
      ...
      for (; numit < maxit2; ++numit) {
        ...
        // printf("cc: %.80f\n", 
        //   sbet1+cbet1+dn1+sbet2+cbet2+dn2+salp1+calp1+slam12+clam12
        // );
        				// printf("  %.80f\n", calp1);
...
//  printf("cc: %.40f %.40f\n", ssig1, csig1);
  ...
  real aa1 = maxx((real)(0), csig1 * ssig2 - ssig1 * csig2);
  real aa2 = csig1 * csig2 + ssig1 * ssig2;
  sig12 = atan2(aa1, aa2);
	// printf("cc: %.40f\n", sig12);
